...
import {createAlas} from '/@/serviceLogic';
...
export class CoreService {
  ...
  // --- alas-mac-pack patch ---
  // Skip createInstaller: upstream runs deploy/installer.py at startup, which is
  // Windows-centric (adb_install -> Windows emulator detection) and fails on
  // macOS, so the backend never started. gui.py handles device connection at
  // runtime; the mac package doesn't need the installer step (git/pip/adb setup
  // is done at build time). Same effect as the old Platypus launcher.
  private eventQueue: Array<CallbackFun> = [createAlas];
  // --- end patch ---
  private stepIndex = 0;
